#!/usr/bin/env python
"""
Test script for Pointer V45 model and training pipeline.
...
Usage:
    ...
    python tests/test_pointer_v45.py
"""
...
def test_model_instantiation():
    """Test that PointerNetworkV45 can be instantiated with various configurations."""
    ...
    model_geolife = PointerNetworkV45(
        num_locations=1187,
        num_users=46,
        d_model=64,
        nhead=4,
...
def test_config_loading():
    ...
    configs = [
        "/data/next_loc_clean_v2/config/models/config_pointer_v45_geolife.yaml",
        "/data/next_loc_clean_v2/config/models/config_pointer_v45_diy.yaml",
    ]
    ...
    for config_path in configs:
        if os.path.exists(config_path):
            ...
            assert 'model' in config, f"Config {config_path} should have 'model' section"
            assert 'training' in config, f"Config {config_path} should have 'training' section"
            
            # Verify model config has required fields
            model_cfg = config['model']
            required_model_keys = ['d_model', 'nhead', 'num_layers', 'dim_feedforward', 'dropout']
...
def test_experiment_output():
    ...
    # Check for recent experiment directories
    experiments_dir = "/data/next_loc_clean_v2/experiments"
    pointer_dirs = [d for d in os.listdir(experiments_dir) if 'pointer_v45' in d]
    
    if pointer_dirs:
        for exp_dir in pointer_dirs:
            exp_path = os.path.join(experiments_dir, exp_dir)
            
            # Check required files
            required_files = ['config.yaml', 'training.log', 'test_results.json', 'val_results.json']
        ...
        print("  ⚠ No Pointer V45 experiment directories found")
...
def run_all_tests():
    """Run all tests."""
    print("=" * 60)
    print("Pointer V45 Model Test Suite")
    print("=" * 60 + "\n")
