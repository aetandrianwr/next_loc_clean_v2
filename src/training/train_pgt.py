"""
Training script for Pointer Generator Transformer (PGT) Model.

This script handles training and evaluation of the PointerGeneratorTransformer model
for next location prediction. It supports both geolife and diy datasets.
...
def collate_fn(batch):
    ...
    for key in ["weekday", "time", "duration", "diff"]:
        x_dict_batch[key] = pad_sequence(
            x_dict_batch[key], batch_first=False, padding_value=0
        )
...
def get_dataloaders(config: Dict) -> Tuple[DataLoader, DataLoader, DataLoader, Dict]:
    ...
    info = {
        ...
        "max_seq_len": max(
            train_ds.max_seq_len, val_ds.max_seq_len, test_ds.max_seq_len
        ),
...
class PGTTrainer:
    """
    Trainer for Pointer Generator Transformer model.
    ...
    def evaluate(self, loader: DataLoader, split: str = "val") -> Dict:
        ...
        metrics["f1"] = f1_score(
            all_true_y.tolist(), all_pred_y_flat, average="weighted", zero_division=0
        )
        ...
        for epoch in range(self.num_epochs):
            ...
            if val_metrics["loss"] < self.best_val_loss:
                ...
                if (
                    ...
                    and self.current_epoch >= self.min_epochs
                ):
    ...
    def _load_checkpoint(self, filename: str):
        ...
        checkpoint = torch.load(
            self.checkpoint_dir / filename, map_location=self.device
        )
...
def init_experiment_dir(
    config: Dict, dataset_name: str, model_name: str = "pgt"
) -> str:
    """
...
def save_results(
    experiment_dir: str, config: Dict, val_perf: Dict, test_perf: Dict, config_path: str
):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Pointer Generator Transformer model for next location prediction"
    )
    parser.add_argument(
    ...
    experiment_dir = init_experiment_dir(config, dataset_name, "pgt")
    print(f"Experiment directory: {experiment_dir}")

    print("=" * 60)
    print("POINTER GENERATOR TRANSFORMER")
    print("=" * 60)
    ...
    print(
        f"  Data loaders: train={len(train_loader)}, val={len(val_loader)}, test={len(test_loader)}"
    )
