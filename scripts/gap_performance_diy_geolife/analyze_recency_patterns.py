...
class RecencyPatternAnalyzer:
    ...
    def plot_recency_comparison(self, results: dict):
        """Plot recency pattern comparison."""
        ...
        bins = np.arange(1, 21)
        ax.hist(diy_pos_filtered, bins=bins, alpha=0.6, label='DIY', color='#2ecc71', density=True)
        ax.hist(geo_pos_filtered, bins=bins, alpha=0.6, label='GeoLife', color='#e74c3c', density=True)
        ax.set_xlabel('Target Position from End (1 = Most Recent)', fontsize=12)
        ax.set_ylabel('Density', fontsize=12)
        ax.set_title('Distribution of Target Position in History\n(Lower = More Recent = Better for Pointer)', fontsize=13)
        ax.legend()
        ...
        ax.set_title('Recency Pattern Metrics\n(Higher = More Predictable with Pointer)', fontsize=13)
        ax.legend()
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ...
        ax.plot(positions, diy_cumulative, 'o-', color='#2ecc71', linewidth=2, markersize=6, label='DIY')
        ax.plot(positions, geo_cumulative, 's-', color='#e74c3c', linewidth=2, markersize=6, label='GeoLife')
        
        ax.axhline(y=50, color='gray', linestyle='--', alpha=0.5, label='50% threshold')
        ax.axhline(y=80, color='gray', linestyle=':', alpha=0.5, label='80% threshold')
        
        ax.set_xlabel('Position from End', fontsize=12)
        ax.set_ylabel('Cumulative % of Targets', fontsize=12)
        ax.set_title('Cumulative Target Position Distribution\n(Higher curve = More targets in recent positions)', fontsize=13)
        ...
        ax.scatter([diy_avg], [ablation_impacts['DIY']], s=300, c='#2ecc71', 
                   marker='o', label='DIY', edgecolors='black', linewidths=2, zorder=5)
        ax.scatter([geo_avg], [ablation_impacts['GeoLife']], s=300, c='#e74c3c', 
        ...
        ax.set_xlabel('Average Recency Pattern Score (%)', fontsize=12)
        ax.set_ylabel('Pointer Removal Impact\n(Relative Acc@1 Drop %)', fontsize=12)
        ax.set_title('Recency Patterns vs Pointer Impact\n(Higher recency → Greater pointer importance)', fontsize=13)
        ax.legend()
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        
        plt.suptitle('Recency Pattern Analysis: Why Pointer Benefits GeoLife More', fontsize=16, fontweight='bold', y=1.02)
        plt.tight_layout()
        plt.savefig(self.output_dir / 'figures' / 'recency_pattern_analysis.png', dpi=300, bbox_inches='tight')
        plt.savefig(self.output_dir / 'figures' / 'recency_pattern_analysis.pdf', bbox_inches='tight')
        plt.close()
    ...
    def plot_predictability_analysis(self, results: dict):
        ...
        ax.hist(diy_scores, bins=bins, alpha=0.6, label=f'DIY (μ={np.mean(diy_scores):.3f})', color='#2ecc71', density=True)
        ax.hist(geo_scores, bins=bins, alpha=0.6, label=f'GeoLife (μ={np.mean(geo_scores):.3f})', color='#e74c3c', density=True)
        ax.set_xlabel('Predictability Score (Recency × Frequency)', fontsize=12)
        ax.set_ylabel('Density', fontsize=12)
        ax.set_title('Location Predictability Score Distribution\n(Higher = Easier for Pointer)', fontsize=13)
        ax.legend()
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ...
        diy_vals = [
            results['diy_pred']['avg_recency_score'] * 100,  # Scale for visualization
        ...
        ax.set_ylabel('Score (×100)', fontsize=12)
        ax.set_title('Predictability Components\n(Scaled ×100 for visualization)', fontsize=13)
        ax.legend()
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'figures' / 'predictability_analysis.png', dpi=300, bbox_inches='tight')
        plt.savefig(self.output_dir / 'figures' / 'predictability_analysis.pdf', bbox_inches='tight')
        plt.close()
