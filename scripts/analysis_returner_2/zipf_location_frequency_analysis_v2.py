...
def plot_zipf(group_stats, c_ref, dataset_name, output_path, max_rank=None):
    ...
    Style matches González et al. (2008) Figure 2d.
    ...
    """
    # Define colors and markers for each group (matching paper style)
    group_styles = {
        5:  {'color': 'black',  'marker': 'o', 'label': '5 loc.',  'fillstyle': 'none'},
        10: {'color': 'red',    'marker': 's', 'label': '10 loc.', 'fillstyle': 'none'},
        30: {'color': 'green',  'marker': 'D', 'label': '30 loc.', 'fillstyle': 'none'},
        50: {'color': 'blue',   'marker': '^', 'label': '50 loc.', 'fillstyle': 'none'}
    }
    ...
    for target_n in sorted(group_stats.keys()):
        ...
        style = group_styles.get(target_n, {'color': 'gray', 'marker': 'x', 'label': f'{target_n} loc.', 'fillstyle': 'none'})
        
        # Apply max_rank filter if specified
        if max_rank is not None:
            stats_df = stats_df[stats_df['rank'] <= max_rank]
        
        # Plot on log-log scale (NO connecting lines, just markers)
        ax_main.loglog(
            stats_df['rank'], 
            stats_df['mean_prob'],
            marker=style['marker'],
    ...
    # Add reference line: c * L^{-1} (solid black line)
    if max_rank is None:
        max_rank = max([stats_df['rank'].max() for stats_df in group_stats.values() if len(stats_df) > 0])
    
    # Extend reference line range
    L_ref = np.logspace(0, np.log10(max_rank * 1.5), 100)
    P_ref = c_ref / L_ref
    ax_main.loglog(L_ref, P_ref, 'k-', linewidth=2.5, label=r'$\sim(L)^{-1}$', alpha=1.0)
    
    # Styling main panel
    ax_main.set_xlabel('$L$', fontsize=28, fontweight='bold')
    ax_main.set_ylabel('$P(L)$', fontsize=28, fontweight='bold')
    ax_main.tick_params(labelsize=20, width=2, length=8, which='major')
    ax_main.tick_params(width=1.5, length=5, which='minor')  # Minor ticks
    ...
    # Legend - larger fonts
    ax_main.legend(loc='lower left', fontsize=16, frameon=False)
    
    # Set axis limits matching the screenshot
    ax_main.set_xlim(0.9, max_rank * 1.3)
    ax_main.set_ylim(0.001, 1.0)
    
    # Inset (linear scale for small ranks) - positioned in top right, smaller to fit properly
    ax_inset = fig.add_axes([0.56, 0.58, 0.32, 0.28])  # [left, bottom, width, height]
    ...
    for target_n in sorted(group_stats.keys()):
        ...
        ax_inset.errorbar(
            ...
            markeredgewidth=1.5,
            fillstyle=style['fillstyle'],
            linewidth=1.5,  # Add connecting lines in inset
            elinewidth=1.5,
            capsize=5,
            capthick=1.5,
            alpha=1.0
    ...
    # Styling inset
    ax_inset.set_xlabel('$L$', fontsize=16, fontweight='bold')
    ax_inset.set_ylabel('$P(L)$', fontsize=16, fontweight='bold')
    ax_inset.tick_params(labelsize=14, width=1.5, length=5, which='major')
    ax_inset.tick_params(width=1.2, length=3, which='minor')  # Minor ticks
    
    # Thicker spines for inset
