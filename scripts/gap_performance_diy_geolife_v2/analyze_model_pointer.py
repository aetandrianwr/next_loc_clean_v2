...
class ModelPointerAnalyzer:
    ...
    def plot_gate_comparison(self, diy_results: dict, geolife_results: dict):
        """Plot gate value comparison with classic scientific style."""
        fig, axes = plt.subplots(1, 3, figsize=(16, 5))
        
        # Gate distribution comparison
        ax = axes[0]
        bins = np.linspace(0, 1, 30)
        ax.hist(diy_results['raw_gates'], bins=bins, alpha=0.6, label=f'DIY (μ={diy_results["avg_gate"]:.3f})', 
                color='white', edgecolor=COLORS['diy'], linewidth=1.5, hatch='///')
        ax.hist(geolife_results['raw_gates'], bins=bins, alpha=0.6, label=f'GeoLife (μ={geolife_results["avg_gate"]:.3f})', 
                color='white', edgecolor=COLORS['geolife'], linewidth=1.5, hatch='...')
        ax.set_xlabel('Gate Value (1=Pointer, 0=Generation)')
        ...
        colors = [COLORS['diy'], COLORS['diy'], COLORS['geolife'], COLORS['geolife']]
        hatches = ['///', '...', '///', '...']
        bars = ax.bar(categories, values, color='white', edgecolor=colors, linewidth=1.5)
        for bar, hatch, color, val in zip(bars, hatches, colors, values):
            bar.set_hatch(hatch)
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'figures' / 'gate_comparison.png', dpi=300, bbox_inches='tight', facecolor='white')
        plt.savefig(self.output_dir / 'figures' / 'gate_comparison.pdf', bbox_inches='tight', facecolor='white')
        plt.close()
        print("  Saved: gate_comparison.png/pdf")
    
    def plot_probability_analysis(self, diy_results: dict, geolife_results: dict):
        """Plot pointer vs generation probability analysis with classic scientific style."""
        ...
        for bar, hatch, color, val in zip(bars, hatches, colors, values):
            ...
            ax.annotate(f'{val:.4f}', xy=(bar.get_x() + bar.get_width()/2, val),
                       xytext=(0, 3), textcoords="offset points", ha='center', fontsize=10)
        ax.set_ylabel('Probability Mass on Target')
        setup_classic_axes(ax)
        ...
        colors = [COLORS['diy']]*3 + [COLORS['geolife']]*3
        hatches = ['///', '...', 'xxx'] * 2
        bars = ax.bar(categories, values, color='white', edgecolor=colors, linewidth=1.5)
        for bar, hatch, color, val in zip(bars, hatches, colors, values):
            bar.set_hatch(hatch)
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'figures' / 'probability_analysis.png', dpi=300, bbox_inches='tight', facecolor='white')
        plt.savefig(self.output_dir / 'figures' / 'probability_analysis.pdf', bbox_inches='tight', facecolor='white')
        plt.close()
    ...
    def plot_pointer_contribution_breakdown(self, diy_results: dict, geolife_results: dict):
        """Plot detailed breakdown showing pointer mechanism contribution with classic style."""
        ...
        ax.hist(diy_gates, bins=bins, alpha=0.5, label='DIY', 
                color='white', edgecolor=COLORS['diy'], linewidth=1.0, hatch='///', density=True)
        ax.hist(geo_gates, bins=bins, alpha=0.5, label='GeoLife', 
                color='white', edgecolor=COLORS['geolife'], linewidth=1.0, hatch='...', density=True)
        
        # Add vertical lines for means
        ax.axvline(np.mean(diy_gates), color=COLORS['diy'], linestyle='--', linewidth=1.5, label=f'DIY μ={np.mean(diy_gates):.3f}')
        ax.axvline(np.mean(geo_gates), color=COLORS['geolife'], linestyle=':', linewidth=1.5, label=f'GeoLife μ={np.mean(geo_gates):.3f}')
        ...
        # Open markers
        ax.scatter(diy_gen[diy_idx], diy_ptr[diy_idx], alpha=0.4, s=25, 
                   facecolors='white', edgecolors=COLORS['diy'], linewidths=0.5, label='DIY', marker='o')
        ax.scatter(geo_gen[geo_idx], geo_ptr[geo_idx], alpha=0.4, s=25, 
                   facecolors='white', edgecolors=COLORS['geolife'], linewidths=0.5, label='GeoLife', marker='s')
        
        # Diagonal line
        ax.plot([0, 0.5], [0, 0.5], 'k--', linewidth=1.0, label='Ptr = Gen')
        
        ax.set_xlabel('Generation Probability on Target')
        ax.set_ylabel('Pointer Probability on Target')
        ax.legend(loc='best', frameon=True, edgecolor='black', fancybox=False)
        setup_classic_axes(ax)
        ...
        diy_ptr_contrib = np.mean(diy_ptr > diy_gen) * 100
        geo_ptr_contrib = np.mean(geo_ptr > geo_gen) * 100
        
        diy_ptr_dom = np.mean(diy_ptr > 0.1) * 100
...
1. GATE VALUES (Pointer Usage):
   ...
   • Both datasets use pointer heavily (gate > 0.5)

2. POINTER PROBABILITY ON TARGET (Target in History):
   • DIY:     {np.mean(diy_ptr):.4f}
   • GeoLife: {np.mean(geo_ptr):.4f}

3. ACCURACY BREAKDOWN:
   • DIY overall:         {diy_results['overall_accuracy']:.1f}%
   • DIY target in hist:  {diy_results['acc_target_in_history']:.1f}%
   • GeoLife overall:     {geolife_results['overall_accuracy']:.1f}%
   • GeoLife in hist:     {geolife_results['acc_target_in_history']:.1f}%
...
CONCLUSION:
The pointer mechanism is MORE CRITICAL for GeoLife
because GeoLife has stronger recency patterns.
"""
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'figures' / 'pointer_contribution_breakdown.png', dpi=300, bbox_inches='tight', facecolor='white')
        plt.savefig(self.output_dir / 'figures' / 'pointer_contribution_breakdown.pdf', bbox_inches='tight', facecolor='white')
        plt.close()
