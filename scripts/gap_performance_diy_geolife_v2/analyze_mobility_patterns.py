...
class MobilityPatternAnalyzer:
    ...
    def plot_target_in_history_comparison(self, diy_rate: float, geolife_rate: float):
        """Plot target-in-history comparison with classic scientific style."""
        ...
        # Classic style: white fill with colored edges and hatches
        bars = ax.bar(datasets, rates, color='white', edgecolor=colors, linewidth=1.5)
        ...
        ax.set_ylabel('Target-in-History Rate (%)')
        ...
        fig.savefig(self.output_dir / 'figures' / 'target_in_history_comparison.png', dpi=300, bbox_inches='tight', facecolor='white')
        fig.savefig(self.output_dir / 'figures' / 'target_in_history_comparison.pdf', bbox_inches='tight', facecolor='white')
        plt.close(fig)
    ...
    def plot_unique_ratio_distribution(self, diy_ratios: list, geolife_ratios: list):
        """Plot distribution of unique location ratios with classic scientific style."""
        plt.close('all')
        ...
        ax.hist(diy_ratios, bins=bins, alpha=0.6, label=f'DIY (μ={np.mean(diy_ratios):.3f})', 
                color='white', edgecolor=COLORS['diy'], linewidth=1.5, hatch='///')
        ax.hist(geolife_ratios, bins=bins, alpha=0.6, label=f'GeoLife (μ={np.mean(geolife_ratios):.3f})', 
                color='white', edgecolor=COLORS['geolife'], linewidth=1.5, hatch='...')
        ax.set_xlabel('Unique Location Ratio (Unique/Total)')
        ...
        for element in ['whiskers', 'caps', 'medians']:
            for item, color in zip(bp[element], colors * 2):
                item.set_color('black')
                item.set_linewidth(1.0)
        
        ax.set_ylabel('Unique Location Ratio')
        setup_classic_axes(ax)
        
        plt.tight_layout()
        fig.savefig(self.output_dir / 'figures' / 'unique_ratio_distribution.png', dpi=300, bbox_inches='tight', facecolor='white')
        fig.savefig(self.output_dir / 'figures' / 'unique_ratio_distribution.pdf', bbox_inches='tight', facecolor='white')
        plt.close(fig)
    ...
    def plot_entropy_comparison(self, diy_entropies: list, geolife_entropies: list, 
        ...
        """Plot entropy comparisons with classic scientific style."""
        plt.close('all')
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        ...
        plt.tight_layout()
        fig.savefig(self.output_dir / 'figures' / 'entropy_comparison.png', dpi=300, bbox_inches='tight', facecolor='white')
        fig.savefig(self.output_dir / 'figures' / 'entropy_comparison.pdf', bbox_inches='tight', facecolor='white')
        plt.close(fig)
        print("  Saved: entropy_comparison.png/pdf")
    
    def plot_comprehensive_comparison(self, diy_results: dict, geolife_results: dict):
        """Plot comprehensive metric comparison with classic scientific style."""
        plt.close('all')
        ...
        bars1 = ax.bar(x - width/2, diy_vals, width, label='DIY', 
                       color='white', edgecolor=COLORS['diy'], linewidth=1.5, hatch='///')
        bars2 = ax.bar(x + width/2, geolife_vals, width, label='GeoLife', 
                       color='white', edgecolor=COLORS['geolife'], linewidth=1.5, hatch='...')
        ...
        plt.tight_layout()
        fig.savefig(self.output_dir / 'figures' / 'comprehensive_comparison.png', dpi=300, bbox_inches='tight', facecolor='white')
        fig.savefig(self.output_dir / 'figures' / 'comprehensive_comparison.pdf', bbox_inches='tight', facecolor='white')
        plt.close(fig)
    ...
    def plot_pointer_benefit_analysis(self, diy_results: dict, geolife_results: dict):
        ...
        target_in_history = [diy_results['target_in_history']['target_in_history_rate'],
                           geolife_results['target_in_history']['target_in_history_rate']]
        bars = ax.bar(datasets, target_in_history, color='white', edgecolor=colors, linewidth=1.5)
        for bar, hatch, color, val in zip(bars, hatches, colors, target_in_history):
            bar.set_hatch(hatch)
            bar.set_edgecolor(color)
        ...
        repetition = [diy_results['unique_ratio']['repetition_rate'] * 100,
                     geolife_results['unique_ratio']['repetition_rate'] * 100]
        bars = ax.bar(datasets, repetition, color='white', edgecolor=colors, linewidth=1.5)
        for bar, hatch, color, val in zip(bars, hatches, colors, repetition):
            bar.set_hatch(hatch)
            bar.set_edgecolor(color)
            ax.annotate(f'{val:.1f}%', xy=(bar.get_x() + bar.get_width()/2, val),
                       xytext=(0, 3), textcoords="offset points", ha='center', fontsize=11)
        ...
        target_last = [diy_results['consecutive']['target_equals_last_rate'],
                      geolife_results['consecutive']['target_equals_last_rate']]
        bars = ax.bar(datasets, target_last, color='white', edgecolor=colors, linewidth=1.5)
        for bar, hatch, color, val in zip(bars, hatches, colors, target_last):
            bar.set_hatch(hatch)
            bar.set_edgecolor(color)
            ax.annotate(f'{val:.1f}%', xy=(bar.get_x() + bar.get_width()/2, val),
                       xytext=(0, 3), textcoords="offset points", ha='center', fontsize=11)
        ...
        # Open markers with colored edges (classic style)
        ax.scatter([target_in_history[0]], [ablation_impacts[0]], s=150, 
                   facecolors='white', edgecolors=COLORS['diy'], 
                   marker='o', label='DIY', linewidths=2, zorder=5)
        ax.scatter([target_in_history[1]], [ablation_impacts[1]], s=150, 
        ...
        ax.plot(x_line, p(x_line), 'k--', linewidth=1.0, label='Trend')
        
        ax.set_xlabel('Target-in-History Rate (%)')
        ...
        plt.tight_layout()
        fig.savefig(self.output_dir / 'figures' / 'pointer_benefit_analysis.png', dpi=300, bbox_inches='tight', facecolor='white')
        fig.savefig(self.output_dir / 'figures' / 'pointer_benefit_analysis.pdf', bbox_inches='tight', facecolor='white')
        plt.close(fig)
