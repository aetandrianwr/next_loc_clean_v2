"""
Hypothesis Testing Experiments: Root Cause Analysis (V2).
...
def experiment_1_stratified_analysis(diy_model, diy_loader, geo_model, geo_loader, device):
    ...
    for idx, name in enumerate(['DIY', 'GeoLife']):
        ...
        axes[idx].bar(x - width, ptr_vals, width, label='Pointer', 
                      color='white', edgecolor=ptr_color, linewidth=1.5, hatch='///')
        axes[idx].bar(x, gen_vals, width, label='Generation', 
                      color='white', edgecolor=gen_color, linewidth=1.5, hatch='\\\\\\')
        axes[idx].bar(x + width, final_vals, width, label='Combined', 
        ...
        create_legend(axes[idx])
        axes[idx].set_ylim(0, 100)
        setup_classic_axes(axes[idx])
        
        # Annotate pointer benefit
        axes[idx].annotate(f'Δ={ptr_vals[0]-gen_vals[0]:.1f}%', 
                          xy=(0, max(ptr_vals[0], gen_vals[0]) + 5),
                          ha='center', fontsize=10)
    
    plt.tight_layout()
    save_figure(fig, OUTPUT_DIR / 'exp1_stratified_analysis', ['png', 'pdf'])
    plt.close()
...
def experiment_2_ablation_simulation(diy_model, diy_loader, geo_model, geo_loader, device):
    ...
    ax.bar(x - width/2, diy_vals, width, label='DIY', color='white', edgecolor=DATASET_COLORS['DIY'], linewidth=1.5, hatch='///')
    ax.bar(x + width/2, geo_vals, width, label='GeoLife', color='white', edgecolor=DATASET_COLORS['GeoLife'], linewidth=1.5, hatch='\\\\')
    ...
    plt.tight_layout()
    save_figure(fig, OUTPUT_DIR / 'exp2_ablation_simulation', ['png', 'pdf'])
    plt.close()
...
def experiment_3_generation_difficulty(diy_model, diy_loader, geo_model, geo_loader, 
    ...
    # Target frequency distribution (log scale) - step plot for classic look
    axes[0, 0].bar(range(len(diy_targets)), sorted(diy_targets.values(), reverse=True), 
                   color='white', edgecolor=DATASET_COLORS['DIY'], linewidth=0.5)
    axes[0, 0].set_xlabel('Location Rank')
    axes[0, 0].set_ylabel('Frequency')
    axes[0, 0].set_yscale('log')
    setup_classic_axes(axes[0, 0])
    
    axes[0, 1].bar(range(len(geo_targets)), sorted(geo_targets.values(), reverse=True), 
                   color='white', edgecolor=DATASET_COLORS['GeoLife'], linewidth=0.5)
    axes[0, 1].set_xlabel('Location Rank')
    axes[0, 1].set_ylabel('Frequency')
    axes[0, 1].set_yscale('log')
    setup_classic_axes(axes[0, 1])
    ...
    axes[1, 0].plot(range(len(diy_cumsum)), diy_cumsum, label='DIY', color=DATASET_COLORS['DIY'], linewidth=1.5, linestyle='-')
    axes[1, 0].plot(range(len(geo_cumsum)), geo_cumsum, label='GeoLife', color=DATASET_COLORS['GeoLife'], linewidth=1.5, linestyle='--')
    ...
    axes[1, 1].bar(x - width/2, [metrics['DIY'][m] for m in metrics_to_plot], width, 
                   label='DIY', color='white', edgecolor=DATASET_COLORS['DIY'], linewidth=1.5, hatch='///')
    axes[1, 1].bar(x + width/2, [metrics['GeoLife'][m] for m in metrics_to_plot], width, 
                   label='GeoLife', color='white', edgecolor=DATASET_COLORS['GeoLife'], linewidth=1.5, hatch='\\\\')
    axes[1, 1].set_ylabel('Value')
    axes[1, 1].set_xticks(x)
    axes[1, 1].set_xticklabels(['Unique\nTargets', 'Top-10\nCov (%)', 'Entropy\nRatio', 'Gini\nCoef'])
    create_legend(axes[1, 1])
    ...
    plt.tight_layout()
    save_figure(fig, OUTPUT_DIR / 'exp3_generation_difficulty', ['png', 'pdf'])
    plt.close()
...
def create_summary_visualization(exp1_results, exp2_results, exp3_metrics):
    ...
    ax1.bar(x - width/2, diy_vals, width, label='DIY', color='white', edgecolor=DATASET_COLORS['DIY'], linewidth=1.5, hatch='///')
    ax1.bar(x + width/2, geo_vals, width, label='GeoLife', color='white', edgecolor=DATASET_COLORS['GeoLife'], linewidth=1.5, hatch='\\\\')
