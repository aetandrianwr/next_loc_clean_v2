"""
Experiment 5: Test Set Manipulation to Prove Causality.
...
def experiment_target_in_history_ablation(diy_model, geo_model, diy_raw, geo_raw, device):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'exp5_target_in_history_ablation.png')
    plt.savefig(OUTPUT_DIR / 'exp5_target_in_history_ablation.pdf')
    plt.close()
...
def experiment_recency_analysis(diy_model, geo_model, diy_raw, geo_raw, device):
    ...
    ax.bar(x - width/2, diy_accs, width, label='DIY', color='#3498db')
    ax.bar(x + width/2, geo_accs, width, label='GeoLife', color='#e74c3c')
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'exp5_recency_effect.png')
    plt.savefig(OUTPUT_DIR / 'exp5_recency_effect.pdf')
    plt.close()
