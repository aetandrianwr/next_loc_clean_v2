...
Usage:
    python scripts/ablation_study/train_ablation.py --config <config_path> --ablation <ablation_name>

Ablation Options:
    - full_model: Complete model with all components
    - no_user_emb: Without user embedding
    - no_time_emb: Without time-of-day embedding
    - no_weekday_emb: Without weekday embedding
    - no_recency_emb: Without recency embedding
    - no_duration_emb: Without duration embedding
    - no_pos_from_end: Without position-from-end embedding
    - no_sinusoidal_pos: Without sinusoidal positional encoding
    - no_temporal: Without all temporal embeddings (time, weekday, recency, duration)
    - no_pointer: Without pointer mechanism (generation only)
    - no_generation: Without generation head (pointer only)
    - no_gate: Without pointer-generation gate (fixed 50-50 blend)
...
ABLATION_CONFIGS = {
    ...
    'no_user_emb': {
        'description': 'Without User Embedding',
        'config': {'use_user_emb': False}
    },
    'no_time_emb': {
        'description': 'Without Time-of-Day Embedding',
        'config': {'use_time_emb': False}
    },
    'no_weekday_emb': {
        'description': 'Without Weekday Embedding',
        'config': {'use_weekday_emb': False}
    },
    'no_recency_emb': {
        'description': 'Without Recency Embedding',
        'config': {'use_recency_emb': False}
    },
    'no_duration_emb': {
        'description': 'Without Duration Embedding',
        'config': {'use_duration_emb': False}
    },
    'no_pos_from_end': {
        'description': 'Without Position-from-End Embedding',
        'config': {'use_pos_from_end_emb': False}
    },
    'no_sinusoidal_pos': {
        'description': 'Without Sinusoidal Positional Encoding',
        'config': {'use_sinusoidal_pos': False}
    },
    'no_temporal': {
        'description': 'Without All Temporal Features',
        'config': {
            'use_time_emb': False,
            'use_weekday_emb': False,
            'use_recency_emb': False,
            'use_duration_emb': False
        }
    },
    'no_pointer': {
        'description': 'Without Pointer Mechanism (Generation Only)',
        'config': {'use_pointer': False}
    },
    'no_generation': {
        'description': 'Without Generation Head (Pointer Only)',
        'config': {'use_generation': False}
    },
    'no_gate': {
        'description': 'Without Pointer-Generation Gate',
...
class AblationTrainer:
    ...
    def _setup_logging(self):
        ...
        self.logger.info("=" * 60)
        self.logger.info(f"ABLATION STUDY: {self.ablation_name}")
        self.logger.info("=" * 60)
        ablation_desc = ABLATION_CONFIGS.get(self.ablation_name, {}).get('description', 'Unknown')
        self.logger.info(f"Ablation: {ablation_desc}")
    ...
    def train(self) -> Tuple[Dict, Dict]:
        ...
        self.logger.info(f"  F1:     {test_metrics['f1']*100:.2f}%")
        self.logger.info("=" * 60)
...
def init_experiment_dir(config: Dict, dataset_name: str, ablation_name: str) -> str:
    ...
    experiment_name = f"ablation_{dataset_name}_{ablation_name}_{timestamp}"
    experiment_dir = os.path.join(experiment_root, "ablation_study", experiment_name)
...
def save_results(experiment_dir: str, config: Dict, val_perf: Dict, test_perf: Dict, 
                 ablation_name: str, ablation_config: Dict):
    """Save all results."""
    # Save configuration
    ...
    ablation_info = {
        ...
        'description': ABLATION_CONFIGS.get(ablation_name, {}).get('description', 'Unknown'),
        'ablation_config': ablation_config,
    }
    with open(os.path.join(experiment_dir, "ablation_info.json"), "w") as f:
        json.dump(ablation_info, f, indent=2)
    
    # Save validation results
    val_results = {k: float(v) if isinstance(v, (np.floating, np.integer, float)) else v 
                   for k, v in val_perf.items()}
    with open(os.path.join(experiment_dir, "val_results.json"), "w") as f:
        json.dump(val_results, f, indent=2)
    
    # Save test results
    test_results = {k: float(v) if isinstance(v, (np.floating, np.integer, float)) else v 
...
def main():
    ...
    # Get ablation config
    ablation_config = ABLATION_CONFIGS[args.ablation]['config']
    
    # Initialize experiment directory
    experiment_dir = init_experiment_dir(config, dataset_name, args.ablation)
    print(f"Experiment directory: {experiment_dir}")
    
    print("=" * 60)
    print(f"ABLATION STUDY: {args.ablation}")
    print(f"Description: {ABLATION_CONFIGS[args.ablation]['description']}")
    print("=" * 60)
    ...
    # Save results
    save_results(experiment_dir, config, val_metrics, test_metrics, args.ablation, ablation_config)
    
    print(f"\n" + "=" * 60)
    print(f"ABLATION RESULTS - {dataset_name.upper()} - {args.ablation}")
    print("=" * 60)
