#!/usr/bin/env python3
"""
Wrapper script for MHSA training that ensures val_results.json is properly saved.

This wrapper calls the original train_MHSA.py and then re-evaluates on validation set
to save complete val_results.json.

Usage:
    python scripts/ht_grid_search/train_MHSA_wrapper.py --config config/path.yaml
"""
...
def find_latest_experiment(experiment_root, dataset_name, model_name="MHSA"):
...
def evaluate_on_validation(config, experiment_dir, device):
    ...
    # Load model
    model = MHSA(config=config, total_loc_num=config.total_loc_num).to(device)
...
def main():
    parser = argparse.ArgumentParser(description="Train MHSA model with proper val_results saving")
    parser.add_argument("--config", type=str, required=True, help="Path to config YAML file")
    args = parser.parse_args()
    
    # First run the original training script
    import subprocess
    train_script = str(PROJECT_ROOT / "src" / "training" / "train_MHSA.py")
    ...
    if not experiment_dir:
        ...
        experiment_dir = find_latest_experiment(config.experiment_root, dataset_name, "MHSA")
