...
Usage:
    python scripts/ht_grid_search/run_optuna_ht.py --model pgt --n_jobs 4
    python scripts/ht_grid_search/run_optuna_ht.py --model MHSA --n_jobs 4
    python scripts/ht_grid_search/run_optuna_ht.py --model LSTM --n_jobs 4
    python scripts/ht_grid_search/run_optuna_ht.py --model all --n_jobs 4
    
    
    python scripts/ht_grid_search/run_optuna_ht.py --model pgt --n_jobs 4 --n_trials 10 --max_epochs 6
...
CONFIG_DIR = PROJECT_ROOT / "config" / "ht_grid_search"
...
def create_pgt_config(
    ...
    config_name = f"optuna_pgt_{dataset}_prev{prev_day}_d{d_model}_h{nhead}_l{num_layers}_ff{dim_feedforward}_lr{lr}_t{trial_id}.yaml"
...
def create_mhsa_config(
    ...
    # Generate config filename
    config_name = f"optuna_MHSA_{dataset}_prev{prev_day}_emb{base_emb_size}_h{nhead}_l{num_encoder_layers}_ff{dim_feedforward}_lr{lr}_t{trial_id}.yaml"
    config_path = CONFIG_DIR / config_name
...
def create_lstm_config(
    ...
    # Generate config filename
    config_name = f"optuna_LSTM_{dataset}_prev{prev_day}_emb{base_emb_size}_h{lstm_hidden_size}_l{lstm_num_layers}_ff{dim_feedforward}_lr{lr}_t{trial_id}.yaml"
    config_path = CONFIG_DIR / config_name
...
def create_objective(
    ...
    def objective(trial: optuna.Trial) -> float:
        # Add delay to avoid naming conflicts
        time.sleep(1)
...
def run_hyperparameter_tuning(
    ...
    print(f"n_jobs: {n_jobs}, n_trials per dataset/prev_day: {n_trials}, max_epochs: {max_epochs}")
    print(f"{'='*80}\n")
