...
Features:
- Queue-based parallelism: Jobs picked from queue, immediately starts next when one finishes
- Real-time logging: Prints "Starting" immediately when job is picked up
...
Usage:
    python scripts/ht_grid_search/run_grid_search_v4.py --max_epochs 2 --num_workers 2 --limit 6 --model pgt # Test run
    python scripts/ht_grid_search/run_grid_search_v4.py --max_epochs 50 # Full run
    python scripts/ht_grid_search/run_grid_search_v4.py --max_epochs 50 --model pgt  # Only PGT
    python scripts/ht_grid_search/run_grid_search_v4.py --max_epochs 50 --model MHSA  # Only MHSA
    python scripts/ht_grid_search/run_grid_search_v4.py --max_epochs 50 --model LSTM  # Only LSTM
    
    A100
    python scripts/ht_grid_search/run_grid_search_v4.py --max_epochs 50 --num_workers 20 --model pgt --delay 1
...
def run_training_job(job: TrainingJob, job_idx: int) -> Dict:
    ...
    print(f"[Job {job_idx}] {status}: {job.model_type} on {job.dataset} prev{job.prev_days} ({elapsed:.1f}s)")
...
def run_worker(job_queue: Queue, worker_id: int, val_csv_path: str, test_csv_path: str,
               model_type: str, start_delay: float, completed_count: List[int], total_jobs: int):
    ...
    while True:
        try:
            job_idx, job = job_queue.get(timeout=1)
        except:
            break
        ...
        # Run the job
        result = run_training_job(job, job_idx)
        ...
        if result["success"]:
            ...
            # Write to CSVs
            append_to_csv(val_csv_path, model_type, job_obj, experiment_dir, val_metrics, num_params)
            append_to_csv(test_csv_path, model_type, job_obj, experiment_dir, test_metrics, num_params)
        else:
            print(f"WARNING: Job failed for {job_obj.config_path}")
            # Write empty results
            append_to_csv(val_csv_path, model_type, job_obj, "", {}, 0)
            append_to_csv(test_csv_path, model_type, job_obj, "", {}, 0)
        
        completed_count[0] += 1
        print(f"Progress: {completed_count[0]}/{total_jobs} jobs completed")
        
        job_queue.task_done()


def run_queue_based_pool(
    ...
    """Run jobs with queue-based parallelism - picks jobs from queue as workers become available."""
    ...
    for idx, job in enumerate(jobs):
        job_queue.put((idx + 1, job))
    
    # Add sentinel values to stop workers
    for _ in range(num_workers):
        job_queue.put((None, None))
    
    # Shared completed counter
    completed_count = [0]
    ...
    for i in range(num_workers):
        worker = threading.Thread(
            target=run_worker,
            args=(job_queue, i, val_csv_path, test_csv_path, model_type, 
                  i * delay_seconds, completed_count, len(jobs)),
        )
        worker.start()
...
def main():
    parser = argparse.ArgumentParser(description="Grid Search Hyperparameter Tuning (Queue-Based Parallelism)")
    ...
    parser.add_argument("--delay", type=float, default=5.0, help="Delay between parallel job starts (seconds)")
    ...
    print("="*60)
    print("Grid Search Hyperparameter Tuning (Queue-Based Parallelism)")
    print("="*60)
    ...
    for model_type in models_to_run:
        ...
        print(f"\n{'='*60}")
        print(f"Starting {model_type} Grid Search (Queue-Based)")
        print(f"Total jobs: {len(jobs)}")
        print(f"Val CSV: {val_csv_path}")
        print(f"Test CSV: {test_csv_path}")
        print(f"{'='*60}")
        
        run_queue_based_pool(
            jobs,
            args.num_workers,
            args.delay,
            str(val_csv_path),
