...
Features:
- Continuous worker pool: Maintains N active jobs continuously
- When a job finishes, immediately starts the next one
- CSV logging of results (val and test)
- Config files stored in config/ht_grid_search/
- Model selection flag to choose which model to tune

Usage:
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 2 --limit 2 # Test run
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 50 # Full run
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 50 --model pointer_v45  # Only PointerV45
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 50 --model MHSA  # Only MHSA
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 50 --model LSTM  # Only LSTM
    
    A100
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 50 --num_workers 32 --model pointer_v45 --delay 1
    
    A100
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 50 --num_workers 32 --model MHSA --delay 1
    
    A100
    python scripts/ht_grid_search/run_grid_search_v3.py --max_epochs 50 --num_workers 32 --model LSTM --delay 1
"""
...
def run_training_job(job: TrainingJob, results_queue: queue.Queue, job_idx: int):
    ...
    if job.model_type == "pointer_v45":
        ...
        script = "scripts/ht_grid_search/train_MHSA_wrapper.py"
    else:
        script = "scripts/ht_grid_search/train_LSTM_wrapper.py"
...
def run_continuous_pool(
    ...
    """Run jobs with continuous worker pool - maintain N active jobs at all times."""
    
    print(f"\n{'='*60}")
    print(f"Running {len(jobs)} {model_type} jobs with continuous pool of {num_workers} workers")
    print(f"{'='*60}")
    ...
    for idx, job in enumerate(jobs):
        job_queue.put((job, idx))
    
    def worker_loop():
        """Continuous worker that picks up jobs from queue."""
        nonlocal write_header
        
        while True:
            try:
                # Get next job (non-blocking with timeout)
                job, job_idx = job_queue.get(timeout=1)
            except queue.Empty:
                # No more jobs
                break
            
            # Add delay before starting (except for first jobs)
            if completed_count[0] > 0:
                time.sleep(delay_seconds)
            
            # Run the job
            run_training_job(job, results_queue, job_idx)
            ...
            try:
                ...
                with write_lock:
                    ...
                    if result["success"]:
                        ...
                        # Write to CSVs
                        write_result_to_csv(val_csv_path, model_type, job_obj, experiment_dir, val_metrics, num_params, write_header)
                        write_result_to_csv(test_csv_path, model_type, job_obj, experiment_dir, test_metrics, num_params, write_header)
                        write_header = False
                    else:
                        print(f"WARNING: Job failed for {job_obj.config_path}")
                        # Write empty results
                        write_result_to_csv(val_csv_path, model_type, job_obj, "", {}, 0, write_header)
                        write_result_to_csv(test_csv_path, model_type, job_obj, "", {}, 0, write_header)
                        write_header = False
                    
                    completed_count[0] += 1
                    print(f"Progress: {completed_count[0]}/{len(jobs)} jobs completed")
            except queue.Empty:
                print("WARNING: Failed to get result from queue")
    ...
    print(f"\n{model_type} Grid Search Complete! Total: {completed_count[0]}/{len(jobs)} jobs")


def main():
    parser = argparse.ArgumentParser(description="Grid Search Hyperparameter Tuning (Continuous Pool)")
    ...
    parser.add_argument("--model", type=str, default="all", choices=["all", "pointer_v45", "MHSA", "LSTM"],
                        help="Model to tune: 'pointer_v45', 'MHSA', 'LSTM', or 'all' (default: all)")
    ...
    print("="*60)
    print("Grid Search Hyperparameter Tuning (Continuous Pool)")
    print("="*60)
    ...
    for model_type in models_to_run:
        ...
        print(f"\n{'='*60}")
        print(f"Starting {model_type} Grid Search (Continuous Pool)")
