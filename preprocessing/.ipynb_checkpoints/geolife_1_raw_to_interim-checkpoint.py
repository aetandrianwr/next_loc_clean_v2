...
def process_raw_to_intermediate(config):
    ...
    sp, locs = sp.as_staypoints.generate_locations(
        ...
        distance_metric=loc_config["distance_metric"],
        agg_level=loc_config["agg_level"],
        n_jobs=-1
    )
    ...
    interim_stats_file = os.path.join(interim_path, f"interim_stats_eps{epsilon}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Geolife preprocessing - Script 1: Raw to Intermediate"
    )
    parser.add_argument(
        "--config",
        type=str,
