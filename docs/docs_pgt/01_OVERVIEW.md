# Pointer Generator Transformer - Comprehensive Documentation

## Executive Summary

**Pointer Generator Transformer** (PointerGeneratorTransformer) is a sophisticated deep learning model designed for the **next location prediction** task in human mobility analytics. The model predicts where a user will go next based on their historical location visits and temporal context.
...
|-------|---------------|-----------|----------------|
| **Pointer Generator Transformer** | **53.97%** | **56.89%** | Hybrid pointer-generation |
| MHSA (Transformer) | 29-31% | 53% | Pure generation |
| LSTM | 29.73% | 51.74% | Recurrent without pointer |
| Markov | ~25% | ~45% | No deep learning |

The Pointer Generator Transformer significantly outperforms baselines, especially on the GeoLife dataset where the improvement is **~24%** over MHSA.
...
Implementation: next_loc_clean_v2/src/models/proposed/pgt.py
